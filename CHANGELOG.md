...
* Apply co-author bylines to feeds on a default install, instead of only when a site opted into `coauthors_auto_apply_template_tags`. Feed markup is generated by WordPress core rather than the theme, so it does not need the same opt-in that theme template tags do, and until now every feed item was credited to whichever WordPress user published the post rather than to the assigned co-author

  **This changes published output.** Sites that had not opted in will see feed bylines change on upgrade, across every item currently in their feed window. To keep the previous behaviour, add:

  ```php
  add_filter( 'coauthors_filter_feed_authors', '__return_false' );
  ```

* Stop double-encoding co-author names inside `dc:creator`. The name was escaped before being placed in a CDATA section, where content is literal, so a display name such as `Smith & Jones` reached feed readers as `Smith &amp; Jones`
* Pass feed bylines through `ent2ncr()`, restoring the entity normalization core applies to `the_author` at priority 8 and which was being bypassed. This is visible in Atom feeds, where the byline is rendered outside CDATA
...
[Unreleased]: https://github.com/automattic/co-authors-plus/compare/4.1.1...develop
