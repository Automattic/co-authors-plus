...
class CoAuthors_Plus {
	 ...
	 * - Programmatic multi-author queries using `author` (comma-separated IDs)
	 *   or `author__in` (array of IDs).
	 *
	 * @since 3.6.0
	 *
	...
	protected function is_author_query( WP_Query $query ): bool {
		...
		// `author` with a comma-separated list of IDs, e.g. '1,2,3'.
	 ...
	 *
	 * Returns an empty array when the query is a standard single-author archive
	 * (those are handled by the existing author_name / single-ID code path).
	 *
	 * @since 3.6.0
	 *
	...
	public function posts_where_filter( $where, $query ): string {
		...
		if ( $this->is_author_query( $query ) ) {
			// For programmatic multi-author queries (author__in or comma-separated author IDs),
			// resolve each author ID to its co-author taxonomy terms and rewrite the WHERE
			// clause so posts co-authored via the taxonomy are also found.
			if ( ! $query->is_author() ) {
			...
			if ( ! empty( $terms ) ) {
				...
				if ( '\d+' !== $id && false === get_user_by( 'id', $id ) ) {
					$id = '\d+';
				}
